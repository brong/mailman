...
def compute_header_hash(msg):
    """Compute SHA-256 header hash per DKIM2 spec-06 §6.2.
...
def compute_body_recipe(current_lines, previous_lines):
    ...
    - {"c": [start, end]}: copy this range of lines from the current body
      (1-based, inclusive)
    - {"d": [line, ...]}: literal lines from the previous body
...
def compute_header_recipe(current_headers, previous_headers):
    ...
    ({"c": [i, j]} copies by bottom-up index, {"d": [...]} restores literal
    values),
...
class MessageInstanceIngress:
    ...
    def process(self, mlist, msg, msgdata):
        ...
        if existing_version > 0:
            ...
            # The baseline decision is Mailman's own bookkeeping and is logged.
            ...
            if matched != existing_version:
                ...
                if b_matched == existing_version:
                    snap_src = baseline
                    log.debug('Existing Message-Instance m=%d matches the '
                              'unstamped baseline', existing_version)
                else:
            ...
            return
        # No MI headers present — add m=1 documenting the current state.
