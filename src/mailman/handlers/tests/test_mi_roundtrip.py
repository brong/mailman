...
class TestExternalMIWithStamp(unittest.TestCase):
    ...
    def _external_mi_message(self, include_midh_in_m1):
        ...
        # The message Mailman actually receives: the external MI is present,
        # and the message store stamps Message-ID-Hash (unless the signer
        # included its own).
        ...
        if include_midh_in_m1:
            ...
            # Mailman's stamp.
            msg['Message-ID-Hash'] = 'ZZZZZZZZZZZZZZZZZZZZZZZZZZZ'
    ...
    def test_signed_message_id_hash_is_preserved(self):
        # The signer included Message-ID-Hash in m=1. verify() matches as-is,
        # so the snapshot keeps it — stripping would (wrongly) break the hash.
        ...
        self.assertIn('Message-ID-Hash', snap,
                      'a Message-ID-Hash that is part of m=1 must NOT be '
                      'stripped')
        self.assertEqual(_b64(compute_header_hash(snap)), _b64(ext_h))
